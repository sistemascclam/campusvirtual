...
export default function Leccion() {
    ...
    const { keyword, identificador } = router.query
    ...
    const handleAllowNext = async () => {
        ...
        if (leccionData?.progress?.advance == 100 || leccionData?.leccion?.orden < leccionData?.progress?.leccion?.orden) {
            ...
            if (allowNext) {
                const axiosnextreq = await axios.post(`/api/curso/${keyword}/leccion/action`, {
                    progressId: leccionData?.progress?.id,
                    newlection: leccionData?.leccion?.id,
                });
                const { status } = axiosnextreq
                if (status == 202) {
                    router.push(`/curso/${keyword}/leccion?identificador=${ide}`)
                }
            }
                        ...
                        <div className='flex flex-wrap md:flex-nowrap justify-between gap-4'>
                            ...
                            <div className='w-full md:w-3/12 px-3 md:px-0'>
                                <Disclosure>
                                    {({ open }) => (
                                        <>
                                            <Disclosure.Button className="bg-slate-800 focus:outline-none flex items-center w-full justify-between rounded-2xl shadow-lg bg-purple-100 px-3 py-2 text-left text-lg font-medium text-purple-900 hover:bg-purple-200 focus-visible:ring focus-visible:ring-purple-500 focus-visible:ring-opacity-75">
                                            ...
                                            <Disclosure.Panel className="mt-2 p-4 text-base text-gray-200 bg-slate-800 rounded-2xl shadow-lg">
                                                {
