...
export default function Curso() {
    ...
    //const [_idprogress, _setIdprogress] = useState(0)
    ...
    useEffect(() => {
        ...
        if (session.data != null){ //BD
            ...
            fetch('/api/public/getProgress/0|.|'+_auxIdCurso)
            .then(response => response.json())
            .then(data =>  _setBtnInscription(data[0].data) );
    ...
    const changeStatus = () => {
        if(btnInscription == 'Inscribirse ahora'){
            _setBtnInscription(btnInscription == 'Inscribirse ahora' ? 'Ir al curso' : 'Inscribirse ahora');
