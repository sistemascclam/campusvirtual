    ...
    <header>
      <div className="mx-auto">
        <nav className="bg-darkblue flex justify-center lg:justify-start items-center  ">
          ...
          {
            user ?
              <button onClick={() => logout()} type="button" className="text absolute top-7 right-10 text-white bg-purple bg-center bg-cover rounded-full h-9 w-9 font-bold">{(user.name ? user.name : user.email).toLocaleUpperCase().substring(0, 1)}</button>
              :
              <Link href="/inicio-sesion" >
                <a className="text absolute top-7 right-10 text-white bg-center bg-cover rounded-full h-9 w-9 font-bold text-xs text-center">Inicia Sesión</a>
              </Link>
          }
        </nav>
      </div>
